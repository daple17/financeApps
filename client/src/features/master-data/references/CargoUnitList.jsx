...
export default function CargoUnitList() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        setLoading(true);
        const data = await masterDataService.getCargoUnits({
          search,
          status: filterStatus
        });
        setItems(data);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">Cargo Unit</h1>
            <p className="text-sm text-slate-400">Kelola satuan kargo / kemasan barang</p>
          </div>
        </div>
        <Button 
          onClick={() => navigate('/master-data/cargo-units/create')}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="p-4 bg-slate-800/50">
          <div className="text-sm text-slate-400 font-medium mb-1 uppercase">TOTAL CARGO UNIT</div>
          <div className="text-2xl font-bold text-white">{summary.total}</div>
        </Card>
        <Card className="p-4 bg-slate-800/50 border-emerald-500/20">
          <div className="text-sm text-emerald-400 font-medium mb-1 uppercase">ACTIVE CARGO UNIT</div>
                ...
                items.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-800/20 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right">
                      <TableActionMenu
                        onEdit={() => navigate(`/master-data/cargo-units/${item.id}/edit`)}
                      />
                    </td>
                  </tr>
                ))
