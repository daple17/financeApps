...
export default function VehicleTypeForm() {
  ...
  useEffect(() => {
    if (isEdit) {
      const fetchItem = async () => {
        try {
          const data = await masterDataService.getVehicleTypeById(id);
          setFormData({
            code: data.code || '',
            name: data.name || '',
            capacity_weight_kg: data.capacity_weight_kg || '',
            capacity_volume_cbm: data.capacity_volume_cbm || '',
            description: data.description || '',
            status: data.status || 'ACTIVE'
          });
        } catch (err) {
          setError('Gagal memuat data vehicle type');
        } finally {
          setLoading(false);
        }
      };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(null);
      if (isEdit) {
        await masterDataService.updateVehicleType(id, formData);
      } else {
        await masterDataService.createVehicleType(formData);
      }
      navigate('/master-data/vehicle-types');
    } catch (err) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-4xl mx-auto pb-12">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">
              {isEdit ? 'Edit Vehicle Type' : 'Tambah Vehicle Type'}
            </h1>
            <p className="text-sm text-slate-400">
              {isEdit ? formData.name : 'Buat referensi tipe kendaraan baru'}
            </p>
          </div>
        </div>
      ...
      <Card>
        <div className="p-5 border-b border-slate-800 bg-slate-900/50">
          <h3 className="text-sm font-bold text-white uppercase tracking-wider">Informasi Tipe Kendaraan</h3>
        </div>
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
          ...
          <Input 
            ...
            onChange={handleChange} 
            required 
            placeholder="Misal: CDE (Colt Diesel Engkel)" 
          />
          ...
          <Input 
            ...
            type="number"
            value={formData.capacity_weight_kg} 
            onChange={handleChange} 
