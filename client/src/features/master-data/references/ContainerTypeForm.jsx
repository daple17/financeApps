...
export default function ContainerTypeForm() {
  const { id } = useParams();
  ...
  useEffect(() => {
    if (isEdit) {
      const fetchItem = async () => {
        try {
          const data = await masterDataService.getContainerTypeById(id);
          setFormData({
            code: data.code || '',
            name: data.name || '',
          ...
          setError('Gagal memuat data container type');
        } finally {
          setLoading(false);
        }
      };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(null);
      if (isEdit) {
        await masterDataService.updateContainerType(id, formData);
      } else {
        await masterDataService.createContainerType(formData);
      }
      navigate('/master-data/container-types');
    } catch (err) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-4xl mx-auto pb-12">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">
              {isEdit ? 'Edit Container Type' : 'Tambah Container Type'}
            </h1>
            <p className="text-sm text-slate-400">
              {isEdit ? formData.name : 'Buat referensi tipe container baru'}
            </p>
          </div>
        </div>
      ...
      <Card>
        <div className="p-5 border-b border-slate-800 bg-slate-900/50">
          <h3 className="text-sm font-bold text-white uppercase tracking-wider">Informasi Tipe Container</h3>
        </div>
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
          ...
          <Input 
            ...
            onChange={handleChange} 
            placeholder="Misal: General Purpose" 
          />
