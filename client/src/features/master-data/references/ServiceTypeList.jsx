...
export default function ServiceTypeList() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        setLoading(true);
        const data = await masterDataService.getServiceTypes({
          search,
          status: filterStatus
        });
        setItems(data);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400">Kelola referensi tipe layanan</p>
          </div>
        </div>
        <Button 
          onClick={() => navigate('/master-data/service-types/create')}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="p-4 bg-slate-800/50">
          <div className="text-sm text-slate-400 font-medium mb-1 uppercase">TOTAL SERVICE TYPE</div>
          <div className="text-2xl font-bold text-white">{summary.total}</div>
        </Card>
        <Card className="p-4 bg-slate-800/50 border-emerald-500/20">
          <div className="text-sm text-emerald-400 font-medium mb-1 uppercase">ACTIVE SERVICE TYPE</div>
                  ...
                  <tr key={item.id} className="hover:bg-slate-800/20 transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm text-slate-400">
                      <div className="flex flex-wrap gap-1">
                        {item.applicable_job_types?.map(type => (
                          <span key={type} className="bg-slate-800 px-2 py-0.5 rounded text-xs">
                            {type}
                          </span>
                        ))}
                      </div>
                    ...
                    <td className="px-4 py-3 text-right">
                      <TableActionMenu
                        onEdit={() => navigate(`/master-data/service-types/${item.id}/edit`)}
                      />
                    </td>
                  </tr>
                ))
