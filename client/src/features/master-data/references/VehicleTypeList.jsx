...
export default function VehicleTypeList() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        setLoading(true);
        const data = await masterDataService.getVehicleTypes({
          search,
          status: filterStatus
        });
        setItems(data);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">Vehicle Type</h1>
            <p className="text-sm text-slate-400">Kelola referensi tipe kendaraan logistik</p>
          </div>
        </div>
        <Button 
          onClick={() => navigate('/master-data/vehicle-types/create')}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="p-4 bg-slate-800/50">
          <div className="text-sm text-slate-400 font-medium mb-1 uppercase">TOTAL VEHICLE TYPE</div>
          <div className="text-2xl font-bold text-white">{summary.total}</div>
        </Card>
        <Card className="p-4 bg-slate-800/50 border-emerald-500/20">
          <div className="text-sm text-emerald-400 font-medium mb-1 uppercase">ACTIVE VEHICLE TYPE</div>
                ...
                items.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-800/20 transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm text-slate-400">
                      {item.capacity_weight_kg ? `${item.capacity_weight_kg} kg` : '-'} / {item.capacity_volume_cbm ? `${item.capacity_volume_cbm} cbm` : '-'}
                    </td>
                    <td className="px-4 py-3 text-sm text-slate-400">
                      {item.description || '-'}
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <TableActionMenu
                        onEdit={() => navigate(`/master-data/vehicle-types/${item.id}/edit`)}
                      />
                    </td>
                  </tr>
                ))
