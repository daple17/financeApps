...
import { Plus, Search, MapPin, Globe } from 'lucide-react';
...
export default function CountryList() {
  ...
  useEffect(() => {
    const fetchCountries = async () => {
      try {
        setLoading(true);
        const data = await masterDataService.getAllCountries({
          search,
          status: filterStatus
        });
        setCountries(data);
  ...
  const summary = {
    ...
    active: countries.filter(p => p.status === 'ACTIVE').length
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">Country</h1>
            <p className="text-sm text-slate-400">Kelola master data negara</p>
          </div>
        </div>
        <Button 
          onClick={() => navigate('/master-data/countries/create')}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="p-4 bg-slate-800/50">
          <div className="text-sm text-slate-400 font-medium mb-1 uppercase">TOTAL COUNTRY</div>
          <div className="text-2xl font-bold text-white">{summary.total}</div>
        </Card>
        <Card className="p-4 bg-slate-800/50 border-emerald-500/20">
          <div className="text-sm text-emerald-400 font-medium mb-1 uppercase">ACTIVE COUNTRY</div>
      ...
      <Card className="p-4 bg-slate-900/50">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1 relative">
            ...
            <input 
              type="text" 
              placeholder="Cari code, name, ISO..." 
              value={search}
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan="6" className="p-12 text-center">
                    ...
                    <div className="text-sm text-slate-500 mt-1">Belum ada master data negara atau kriteria pencarian tidak cocok.</div>
                  </td>
                </tr>
              ) : (
                countries.map((country) => (
                  <tr key={country.id} className="hover:bg-slate-800/20 transition-colors">
                    <td className="px-4 py-3">
                      <span className="font-mono text-sm text-sky-400">{country.country_code}</span>
                    </td>
                    ...
                    <td className="px-4 py-3 text-sm text-slate-300">
                      <div className="flex items-center gap-2">
                        {country.iso2 && <span className="bg-slate-800 px-1.5 py-0.5 rounded text-xs">{country.iso2}</span>}
                        {country.iso3 && <span className="bg-slate-800 px-1.5 py-0.5 rounded text-xs">{country.iso3}</span>}
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border ${
                        ...
                        {country.status === 'ACTIVE' ? 'Aktif' : 'Non-aktif'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <TableActionMenu
                        onEdit={() => navigate(`/master-data/countries/${country.id}/edit`)}
                      />
                    </td>
                  </tr>
                ))
